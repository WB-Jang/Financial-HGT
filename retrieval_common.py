...
# ── 어휘(lexical) 검색: 의존성 없는 최소 BM25 (Dense+Sparse 하이브리드용) ──────────

def korean_tokenize(text):
    """한국어 법령 텍스트용 경량 토크나이저.

    형태소 분석기 없이 어휘 매칭 신호를 뽑기 위해:
    - 숫자(조/항 번호 '25', '3' 등)와 영문 토큰은 그대로,
    - 한글은 문자 2-gram(예: '준법감시인' -> 준법/법감/감시/시인)으로 분해한다.
    한글 2-gram은 조사(을/를/의)가 붙어도 어간 부분이 겹쳐 매칭되므로,
    분석기 없이도 법률 용어의 정확 일치 신호를 비교적 견고하게 포착한다.
    """
    if not isinstance(text, str):
        text = str(text)
    toks = []
    for m in re.findall(r'[가-힣]+|[A-Za-z]+|\d+', text):
        if m[0].isdigit() or m[0].isascii():
            toks.append(m.lower())
        else:  # 한글: 2-gram (한 글자면 그대로)
            toks.extend([m[i:i+2] for i in range(len(m) - 1)] or [m])
    return toks


class BM25:
    """BM25 Okapi. corpus_tokens: List[List[str]]"""
    ...
    def scores(self, query_tokens):
        """질의에 대한 전체 문서 BM25 점수 배열(list of float, 길이 N)."""
        scores = [0.0] * self.N
