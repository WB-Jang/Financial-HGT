...
# 배치 평가: '폭 우선' vs '관련도 우선' 리드 조항 비교 (같은 test 분할)
python assemble_context.py --test_size 300 --pool_n 20
```

배치 평가 지표 (검색 recall이 아니라 **앞단이 질의의 법적 범위를 얼마나 넓게 감싸는가**):
- **lead_cross_laws**: 리드 조항이 참조하는 타법 수 — 폭 우선이 관련도 우선보다 커야 함(폭이 실제로 넓어짐).
- **reach_coverage**: 리드 조항의 (자법 ∪ 참조 graph법)이 질의의 정답 법령을 덮는 비율 — 폭 우선이 높아야
  함(앞단이 질의의 법적 범위를 구조적으로 더 넓게 포괄).
- **breadth_lead_relrank**: 폭-리드가 관련도 몇 위였는지 — 낮을수록 관련성을 희생하지 않고 폭을 얻음.

> 최종(본질) 평가는 폭-우선 RAG vs 일반 RAG의 답변 통찰을 LLM 심판으로 비교하는 종단 실험이며, 위
> reach_coverage는 그 전 단계의 가벼운 프록시다.
